...
class ProgramacionServices {
  ...
  async actualizarPlanilla(data) {
    ...
    await Programacion.updateMany(
      { planilla },
      { $set: { conductor, cedula, placa, transporte } }
    ).then((res) => {
      if (res) {
        this.resultado = errorHandler(
          false,
          200,
          `${res.matchedCount} Fields Afecteds`,
          res
        );
      } else {
        this.resultado = errorHandler(true, 400, 'Not data Found', res);
    ...
    await Programacion.deleteMany({ planilla: planilla }).then((res) => {
      if (res.deletedCount > 0) {
        this.resultado = errorHandler(
          false,
          200,
          `${res.deletedCount} field Deleted`,
          planilla
        );
      } else {
        this.resultado = errorHandler(
  ...
  async eliminarChasisDePlanilla(planilla, data) {
    ...
    await Programacion.deleteOne({ id_chasis: id_chasis.data[0]._id }).then(
      (r) => {
        if (r.deletedCount > 0) {
          this.resultado = errorHandler(
            false,
            200,
            `${r.deletedCount} fields deleted`,
            chasis
          );
        } else {
          this.resultado = errorHandler(
            true,
            204,
            `${chasis} exist But, Not scheduled`,
            chasis
          );
        }
      }
